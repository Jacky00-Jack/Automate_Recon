...
output = []
for i in range(2, 4):
    combos = iter.combinations(df['Value'], i)
    print(combos)
    for combo in combos:
        combo_sum = abs(sum(combo))
        if combo_sum == 10:
            output.append(combo)
            df = df[df['Value'].isin(combo) == False]
for i in range(4, 6):
    combos = iter.combinations(df['Value'], i)
    print(combos)
    for combo in combos:
...
df['Value'] = np.where(df['Item Type'].str.contains('S CR'),-1*df['Value'], np.where(df['Item Type'].str.contains('L DR'),-1*df['Value'],df['Value']))
